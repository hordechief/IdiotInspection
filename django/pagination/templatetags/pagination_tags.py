...
def paginate(context, window=DEFAULT_WINDOW, hashtag=''):
    ...
    try:
        ...
        first = set(list(page_range)[:window]) # first = set(page_range[:window])
        last = set(list(page_range)[-window:]) # last = set(page_range[-window:])
        ...
        if current_end < 0:
            current_end = 0
        current = set(list(page_range)[current_start:current_end]) #current = set(page_range[current_start:current_end])
        pages = []
